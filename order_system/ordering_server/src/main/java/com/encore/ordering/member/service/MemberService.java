...
//        return members.stream().map(m -> MemberResponseDto.toMemberResponseDto(m)).collect(Collectors.toList());
        return members.stream().map(MemberResponseDto::toMemberResponseDto).collect(Collectors.toList());
    }
...
//                .build();
        ...
        return memberRepository.save(Member.toEntity(memberCreateReqDto));
    }
    ...
    public Member login(LoginReqDto loginReqDto) throws IllegalArgumentException
    {
        ...
        if (!passwordEncoder.matches(loginReqDto.getPassword(), member.getPassword())) throw new IllegalArgumentException("비밀번호 불일치");
